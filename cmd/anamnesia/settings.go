...
var settings = []setting{
	...
	{Key: "retrieval.recall_max_distance", Kind: kFraction, Def: "0.60", Env: "ANAMNESIA_RECALL_MAX_DISTANCE",
		Doc: "How close a memory must be to a prompt, as a cosine distance where 0 is identical, before that retrieval is tallied as having recalled something on the console. This decides a number you read and nothing about what is returned: raising or lowering it never changes a single hit a session receives. It needs an absolute bar for the same reason the artifact surface does, because a retrieval returns its top K rows whether or not any of them match, so counting non-empty results would only count the corpus being non-empty. Measured on openai/text-embedding-3-small: a paraphrased question matched the memory answering it at 0.528 and a near-verbatim one at 0.153, while questions about something else scored 0.747 and up, so 0.60 sits in a wide gap. Where a reranker also runs it is deliberately not consulted, because its scores are relative to the model and no fixed bar travels between them: the same 0.528 match scored 0.252 on cohere/rerank-v3.5, which a bar set for one model would read as a miss and another as a hit. Recalibrate if you change embedding models, and read the result as a measure rather than a target: prompts about things you have never discussed have nothing to recall, so no install should expect to approach 100%. Ignored while embed.provider is stub, which hashes text into a random unit vector and so puts a prompt about as far from the memory that answers it as from any other; those retrievals are recorded as unmeasured rather than as failures."},
