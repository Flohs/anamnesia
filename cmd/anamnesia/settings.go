...
var settings = []setting{
	...
	{Key: "retrieval.artifact_max_distance", Kind: kFraction, Def: "0.60", Env: "ANAMNESIA_ARTIFACT_MAX_DISTANCE",
		Doc: "How close a published artifact must be to a prompt, as a cosine distance from 0 (identical) to 1, before it is offered alongside the answer. An artifact is a link put in front of someone who did not ask for one, so it has to clear an absolute bar rather than merely rank first: a fused RRF score cannot express that, because it gives the best of an irrelevant pool exactly what it gives the best of a relevant one. 0.60 was measured over 33 real artifacts with openai/text-embedding-3-small: matches a person would call relevant scored 0.32 to 0.63, and the closest match for a prompt about something else scored 0.759, so the band between them is wide and 0.60 sits inside it with no false positives. Raise it toward 0.65 to catch the tail of that relevant range at the cost of the occasional link that is not about the question; lower it to surface only on a close match. Set it to 0 to stop surfacing artifacts on prompts entirely, leaving `anamnesia artifacts` and the session-start list. Recalibrate if you change embedding models: the bands are a property of the model, not of anamnesia."},
	{Key: "retrieval.recall_max_distance", Kind: kFraction, Def: "0.60", Env: "ANAMNESIA_RECALL_MAX_DISTANCE",
		Doc: "How close a memory must be to a prompt, as a cosine distance where 0 is identical, before that retrieval is tallied as having recalled something. This decides a number on the console and nothing about what is returned: raising or lowering it never changes a single hit a session receives. It needs an absolute bar for the same reason the artifact surface does, because a retrieval returns its top K rows whether or not any of them match, so counting non-empty results would count the corpus being non-empty. 0.60 is carried over from retrieval.artifact_max_distance, which was measured on real data with openai/text-embedding-3-small. Recalibrate it if you change embedding models, and read it as a measure rather than a target: prompts about things you have never discussed have nothing to recall, so no install should expect to approach 100%. Ignored while embed.provider is stub, which hashes text into a random unit vector and so puts a prompt about as far from the memory that answers it as from any other; those retrievals are recorded as unmeasured rather than as failures."},
	{Key: "retrieval.recall_min_score", Kind: kFraction, Def: "0.50", Env: "ANAMNESIA_RECALL_MIN_SCORE",
		Doc: "The relevance a reranked hit must reach, 0 to 1, before that retrieval is tallied as having recalled something. Used in place of retrieval.recall_max_distance wherever a reranker ran, because it scored these exact hits against this exact question, which beats the distance between two embeddings. Matches the bar cross-project hits already clear. Only meaningful with rerank.provider set."},
